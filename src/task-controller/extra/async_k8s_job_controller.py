...
async def async_run_jobs():
    task = []
    for i in range(10):
        config = {
            "namespace": "default",
            "job-name": f"hello-skywalker-{i}",
            "python-codes-configmap-name": "hello-skywalker-configmap",
        }
        k8s_job = AsyncJob(job_config=config)
        task.append(k8s_job.run())
    await asyncio.gather(*task)


if __name__ == "__main__":
    asyncio.run(async_run_jobs())
